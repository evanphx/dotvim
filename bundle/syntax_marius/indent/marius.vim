...
let s:syng_strcom = '\<marius\%(Regexp\|RegexpDelimiter' .
      \ '\|String\|StringEscape\|ASCIICode' .
      \ '\|Interpolation\|NoInterpolation\|Comment\|Documentation\)\>'

" Regex of syntax group names that are strings.
let s:syng_string =
      \ '\<marius\%(String\|Interpolation\|NoInterpolation\|StringEscape\)\>'

" Regex of syntax group names that are strings or documentation.
let s:syng_stringdoc =
  \'\<marius\%(String\|Interpolation\|NoInterpolation\|StringEscape\|Documentation\)\>'
...
let s:marius_indent_keywords = '^\s*\zs\<\%(module\|class\|def\|if\|for' .
...
let s:marius_deindent_keywords =
...
" 3. GetMariusIndent Function {{{1
" =========================

function GetMariusIndent()
  " 3.1. Setup {{{2
  " ----------
  ...
  " TODO: this is messy
  if s:Match(v:lnum, s:marius_deindent_keywords)
  ...
  let col = s:Match(lnum, s:marius_indent_keywords)
  if col > 0
    call cursor(lnum, col)
...
"    let ind = indent(lnum) + &sw
  ...
  if s:Match(lnum, s:marius_indent_keywords)
    let ind = msl_ind + &sw
    if s:Match(lnum, s:end_end_regex)
      let ind = ind - &sw
    endif
